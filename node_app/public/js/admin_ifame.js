...
var Tab = {
	addTab: function (title, url) {
		...
		if (parent.$("#tabs #tab_" + index).length == 0) {
			// 添加选项卡
			parent.$('.content_tab li').removeClass('cur');
			...
			parent.$('.content_tab>ul').append(tab);
			// 添加iframe
			parent.$('.iframe').removeClass('cur');
			...
			parent.$('.content_main').append(iframe);
			initScrollShow();
			parent.$('.content_tab>ul').animate({ scrollLeft: window.parent.document.getElementById("tabs").scrollWidth - window.parent.document.getElementById("tabs").clientWidth }, 200, function () {
				initScrollState();
			});
		} else {
			parent.$('#tab_' + index).trigger('click');
		}
		// 关闭侧边栏
		$('#guide').trigger(click);
	},
...
function initScrollState() {
	...
	if (($('.content_tab>ul').scrollLeft() + window.parent.document.getElementById("tabs").clientWidth) >= window.parent.document.getElementById("tabs").scrollWidth) {
